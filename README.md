...
- **Trainer code** (committed): `train.py`, `snake_hl/*` (env, eval, baselines,
  failure_report, html_replay), tests, AGENTS.md, etc.
- **Per-experiment data** (gitignored): `experiments/<exp-name>/` contains
  `policy.py`, `heuristic_notes.md`, `runs/`, `replays/`, `reports/`,
  `trials.jsonl`, `training_curve.{json,html}`. Each experiment is its own
  directory and can have its own `.git/` for experiment-internal versioning.
